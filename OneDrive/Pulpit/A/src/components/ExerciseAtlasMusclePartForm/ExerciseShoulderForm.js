...
const ExerciseShoulderForm = () => {
  const [selectedExercise, setSelectedExercise] = useState(null);
  const navigation = useNavigation();

  const handleExercisePress = (exerciseName) => {
    const selectedExerciseInfo = exercisesData.shoulder.find(
  ...
  return (
    <View style={styles.container}>
      <Header title={"Ćwiczenia barków"} />
      

      {exercisesData.shoulder.map((exercise) => (
...
export default ExerciseShoulderForm;
