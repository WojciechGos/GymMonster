...
const ExerciseArmForm = () => {
  const [selectedExercise, setSelectedExercise] = useState(null);
  const navigation = useNavigation();

  const handleExercisePress = (exerciseName) => {
    const selectedExerciseInfo = exercisesData.arm.find(
  ...
  return (
    <View style={styles.container}>
      <Header title={"Ćwiczenia rąk"} />
      

      {exercisesData.arm.map((exercise) => (
...
export default ExerciseArmForm;
